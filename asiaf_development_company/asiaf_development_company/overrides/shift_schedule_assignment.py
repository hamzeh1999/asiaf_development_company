...
def set_custom_project_from_tool(doc, method):
    ...
    on a new Shift Schedule Assignment from one of two sources (in priority order):

    1. frappe.flags — set by CustomShiftAssignmentTool._bulk_assign
    2. Shift Assignment Tool single doc — direct fallback
    """
    ...
    # ------------------------------------------------------------------
    # 2. Fallback: Shift Assignment Tool single doc
    # ------------------------------------------------------------------
    try:
        tool = frappe.get_cached_doc("Shift Assignment Tool")
        if tool.action == "Assign Shift Schedule" and tool.custom_project:
        ...
        _logger.exception(
            ...
            "Shift Assignment Tool for Shift Schedule Assignment %r.",
            doc.name,
        )